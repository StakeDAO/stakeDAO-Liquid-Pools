...
const STAKE_CAPITAL_DAO_ID = "stake-capital-test24" // This ID must be unique, change it for each new deployment or a revert will occur
...
const SCT_VOTING_PARAMS = ["500000000000000000", "300000000000000000", "86400"] // [supportRequired, minAcceptanceQuorum, voteDuration] 10^16 == 1%
...
module.exports = async () => {
    try {
        ...
        console.log(`SCT Token address: ${sct.address} Transferring SCT to account2 (${account2})...`)
        await sct.transfer(account2, TEST_ACCOUNT_2_SCT_BALANCE)
        console.log(`Account1 (${account1}) SCT balance: ${await sct.balanceOf(account1)} \nAccount2 (${account2}) SCT balance: ${await sct.balanceOf(account2)}`)
